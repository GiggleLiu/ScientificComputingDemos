...
@testset "treeverse" begin
    ...
    function back(x, y̅::Nothing)
        # compute the gradient of the last state
        return [1, 0, 0.0]
    end
    function back(x, y̅)
        ...
        x̅ = copy(x)
        Enzyme.autodiff(Reverse, Const(forward), Duplicated(x, x̅), Duplicated(step_func(x), y̅))
        return x̅
    end

    @test back(randn(3), randn(3)) isa Vector{Float64}

    @testset "treeverse gradient" begin
        x0 = [1.0, 0.0, 0.0]
        for N in [20, 120, 126]
            g_fd = ForwardDiff.gradient(x->rk4(lorentz, x, nothing; t0=0.0, Δt=3e-3, Nt=N)[1], x0)
            g_tv = treeverse(step_func, back, x0; δ=4, N=N)
            @test g_fd ≈ g_tv
        end
    end
end
